        ...
        for o in lista[f]:
            ...
            if _tel in ['fts', 'ftn']:
                outname = hdr0['SITEID'] + re.sub('-', '', hdr0['TELID']) + '_' + \
                          hdr0['instrume'] + '_' + re.sub('-', '', hdr0['DATE-OBS'].split('T')[
                    0]) + '_' + o + '_' + f + '.fits'
            else:
            ...
            for gg in imglist1:
                ...
                out_fits.writeto(re.sub('.fits', '.mask.fits', gg.split('/')[-1]), overwrite=True,
                                 output_verify='fix')
                imglist2 = imglist2 + ',' + output
                imgmask = imgmask + ',' + re.sub('.fits', '.mask.fits', gg.split('/')[-1])
                imglistnoise = imglistnoise + ',' + re.sub('.fits', '.noise.fits', gg.split('/')[-1])
...
# if _tel in ['fts','ftn']:                 line=line+' -IMOUT_BITPIX 16 -IMOUT_BSCALE 1.0 -IMOUT_BZERO 32768.0'
            ...
            dictionary['filename'] = outname.split('/')[-1]
            ...
            ###################    insert in photlco
            ggg = lsc.mysqldef.getfromdataraw(conn, 'photlco', 'filename', outname.split('/')[-1], '*')
            if ggg and force:   lsc.mysqldef.deleteredufromarchive(outname.split('/')[-1], 'photlco',
                                                                   'filename')
            if not ggg or force:
                print('insert')
                print(dictionary)
                ...
                for voce in ggg[0].keys():
                    print('update')
                    try:
                        lsc.mysqldef.updatevalue('photlco', voce, dictionary[voce], outname.split('/')[-1])
                    except:
                        pass
            ...
            ggg = lsc.mysqldef.getfromdataraw(conn, 'photpairing', 'nameout', outname.split('/')[-1], '*')
            if ggg:   lsc.mysqldef.deleteredufromarchive(outname.split('/')[-1], 'photpairing', 'nameout')
            for img in imglist1:
                dictionary = {'namein': img.split('/')[-1], 'nameout': outname.split('/')[-1],
            ...
            for gg in imglist1:
                os.system('rm ' + re.sub('.fits', '.mask.fits', gg.split('/')[-1]))
                os.system('rm ' + re.sub('.fits', '.clean.fits', gg.split('/')[-1]))
                os.system('rm ' + re.sub('.fits', '.sat.fits', gg.split('/')[-1]))
