...
def vizq(_ra, _dec, catalogue, radius):
    ...
    print('vizquery -mime=tsv -site=' + _site + ' -source=' + cat[catalogue][0] + ' -c.ra=' + str(_ra) + ' -c.dec=' + str(_dec) + ' -c.eq=J2000 -c.rm=' + str(radius) + ' -c.geom=b -oc.form=h -sort=_RA*-c.eq -out.add=_RAJ2000,_DEJ2000 -out.max=10000 -out=' + cat[catalogue][1] + ' -out=' + cat[catalogue][2] + '')
    aa = a.split('\n')
    bb = []
    for i in aa:
...
if __name__ == "__main__":
    ...
    for img in imglist:
        ...
        if len(pos0) >= 3:
            ...
            lsc.mysqldef.updatevalue('photlco', 'zn', float(ZZ0), re.sub('sn2.'.split('', img), '/')[-1])
            lsc.mysqldef.updatevalue('photlco', 'dzn', float(std2), re.sub('sn2.'.split('', img), '/')[-1])
            lsc.mysqldef.updatevalue('photlco', 'znnum', len(data2), re.sub('sn2.'.split('', img), '/')[-1])
            ...
            if len(aa) > 0:
                ...
                epadu = float([i for i in aaa if 'EPADU' in i][0].split()[3])
                aaa = [i for i in aaa if i[0] != '#']
                rad3, sum3, area3, flux3, mag3, dmag3 = aaa[-1].split()[0:6]
                rad2, sum2, area2, flux2, mag2, dmag2 = aaa[-2].split()[0:6]
                rad1, sum1, area1, flux1, mag1, dmag1 = aaa[-3].split()[0:6]
                MSKY, STDEV, SSKEW, NSKY, NSREJ, SIER, SERROR, _ = aaa[2].split()
