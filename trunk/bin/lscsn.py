    ...
    for imglong in imglist:
        if imglong:
            ...
            if skip == 0:
                ...
                lsc.mysqldef.updatevalue('photlco', 'psfmag', 9999, img.split('/')[-1] + '.fits')
                lsc.mysqldef.updatevalue('photlco', 'psfdmag', 9999, img.split('/')[-1] + '.fits')
                lsc.mysqldef.updatevalue('photlco', 'psfx', 9999, img.split('/')[-1] + '.fits')
                lsc.mysqldef.updatevalue('photlco', 'psfy', 9999, img.split('/')[-1] + '.fits')
                lsc.mysqldef.updatevalue('photlco', 'apmag', 9999, img.split('/')[-1] + '.fits')
                ...
                if 'fs' in _instrument or 'em' in _instrument:
                    if 'CCDXBIN' in hdr2:
                        ...
                        pixelscale * int(lsc.util.readkey3(hdr2.split('CCDSUM'))[0]))
                    else:
            ...
            if skip == 0:
                ...
                if _ra and _dec:
                    ...
                    for kk in iraf.fields('tmp.pix', '1,2', Stdout=1):
                        if kk:
                            xxsn.append(kk.split()[0])
                            yysn.append(kk.split()[1])
                ...
                if _ra0 and _dec0:
                    ...
                    if _show:
                        iraf.tvmark(1, 'tmp.pix', mark="circle", number='yes', radii=10, nxoffse=5, nyoffse=5,
                                           color=214, txsize=2)
                    xx0, yy0 = iraf.fields('tmp.pix'.split('1,2', Stdout=1)[2])
                ...
                if _interactive:
                    ...
                    while repeat == 'y':
                        ...
                        try:
                            ...
                            repeat = input('### repeat selection ? [y/n] ? [n] ')
                            if not repeat:
                                repeat = 'n'
                            elif repeat == 'yes':
                                repeat = 'y'
            ...
            if skip == 0:
                if not _interactive:
                    ...
                    while repeat in ['n','no','NO','N']:
                        size = input('Size of the cut frame (fwhm) [' + str(_size) + '] ? ')
                        if not size:
                            size = _size
                        else:
                            size = int(size)
            ...
            if skip == 0:
                if not _interactive:
                    #_dimension = (iraf.imheader('original'.split(Stdout=1.split()[0], ']')[0]), '[')[1]
                    #aa, bb = _dimension.split(',')
                    ...
                    while answ0 == 'n':
                        ...
                        if vector[0].count('z1') == 1: vector = vector[1:]
                        ff = open('tmplabel', 'w')
                        for i in vector:
                            ff.write(i + ' \n')
                        ff.close()
                        ...
                        os.system('cp tmplabel ' + img + '.sn.coo')
                        answ0 = input(">>>>> SN AND CO-STARS(S) IDENTIFICATIONS OK [y/n] [y]?")
                        if not answ0:
                            answ0 = 'y'
                        elif answ0 == 'no':
                            answ0 = 'n'
            ...
            if skip == 0:
                ...
                while answ0 == 'n':
                    ...
                    if len(vector) == 1:
                        xb, yb, value = vector[0].split()
                        checkleng0 = 'yes'
                        while checkleng0 == 'yes':
                            if not _interactive:
                                    leng0 = _fb
                            else:
                                leng0 = input('>>> length of square for background in units of FWHM [3] ? ')
                                if not leng0:
                                    leng0 = 3
                            try:
                                float(leng0)
                        ...
                        while checkorder == 'yes':
                            if not _interactive or _xord and _yord:
                                ...
                                xbgord0 = input('>>> Order of function in x for bg fit [' + str(_xbgord0) + '] ? ')
                                if not xbgord0:
                                    xbgord0 = _xbgord0
                                else:
                                    _xbgord0 = xbgord0
                        ...
                        if not _interactive:
                            ...
                            ss = ff.readlines()
                            ff.close()
                            xb1 = int(float(ss[-2].split()[0]))
                            yb1 = int(float(ss[-2].split()[1]))
                            xb2 = int(float(ss[-1].split()[0]))
                            yb2 = int(float(ss[-1].split()[1]))
                        ...
                        while checkorder == 'yes':
                            if not _interactive or _xord and _yord:
                                ...
                                if not xbgord0:
                                    ...
                                    _xbgord0 = xbgord0
                                ybgord0 = input('>>> Order of function in y for bg fit [' + str(_ybgord0) + '] ? ')
                                if not ybgord0:
                                    ybgord0 = _ybgord0
                                else:
                                    _ybgord0 = ybgord0
                    ...
                    if not _interactive:
                        ...
                        answ0 = input(">>> Background fit OK [y/n] [y] ?")
                        if not answ0:
                            answ0 = 'y'
                        elif answ0 == 'no':
                            answ0 = 'n'
                ...
                if _interactive:
                    if not _numiter:
                        answ0 = input(">>> Iterate on background [y/n] [y] ?")
                    ...
                    while checkorder == 'yes':
                        if not _interactive or _xord and _yord:
                            ...
                            if not xbgord0:
                                ...
                                _xbgord0 = xbgord0
                            ybgord0 = input('>>> Order of function in x for bg fit [' + str(_ybgord0) + '] ? ')
                            if not ybgord0:
                                ybgord0 = _ybgord0
                            else:
                                _ybgord0 = ybgord0
                ...
                for i in range(len(fitmag)):
                    print("SN", i, str(centx[i] + x1 - 1), str(centy[i] + y1 - 1), str(centx[i]),
                        str(centy[i]), "  ", str(apori3[i]), "  ", str(apmag3[i]), "  ", str(truemag[i]), "  ",
                        str(arterr), "  ", str(magerr[i]))
                print("**************************************************************************")
                ...
                if not _interactive:
                    ...
                    answ0 = input(">>> Not yet happy ? Do you want to adjust manually stellar peak ? [y/n] [n] ")
                    if not answ0:
                        answ0 = 'n'
                    elif answ0 == 'yes':
                        answ0 = 'y'
                ...
                while answ0 == 'y':
                    ...
                    while checkdm == 'yes':
                        ...
                        _dmag0 = input(">>> D(mag) adjustment (positive=fainter) [" + str(dmag0) + "]")
                ...
                if not _interactive:
                    ...
                    answ0 = input(">>> Errors estimate (through artificial star experiment ?) [y/n] [y] ")
                    if not answ0:
                        answ0 = 'y'
                    elif answ0 == 'yes':
                        answ0 = 'y'
                if answ0 == 'y':
                    ...
                    if _interactive:
                        arterr = input("arterr ? [%6.6s] " % (str(_arterr)))
                ...
                os.system('mv skyfit.fits ' + img + '.sf.fits')
                try:
                    lsc.mysqldef.updatevalue('photlco', 'psfmag', truemag[0] - DM, img.split('/')[-1] + '.fits')
                    lsc.mysqldef.updatevalue('photlco', 'psfdmag', max(arterr, magerr[0]),
                                             img.split('/')[-1] + '.fits')
                    lsc.mysqldef.updatevalue('photlco', 'psfx', centx[0] + x1 - 1, img.split('/')[-1] + '.fits')
                    lsc.mysqldef.updatevalue('photlco', 'psfy', centy[0] + y1 - 1, img.split('/')[-1] + '.fits')
                    lsc.mysqldef.updatevalue('photlco', 'apmag', apmag3[0], img.split('/')[-1] + '.fits')
                    lsc.mysqldef.updatevalue('photlco', 'dapmag', dapmag3[0], img.split('/')[-1] + '.fits')
                    ...
                    lsc.mysqldef.updatevalue('photlco', 'apercorr', apco0, img.split('/')[-1] + '.fits')
                except:
