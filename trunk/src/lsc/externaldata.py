...
def sdss_swarp(imglist,_telescope='spectral',_ra='',_dec='',output='', objname='',survey='sloan',combine_type='MEDIAN', show=False):
    ...
    if survey =='sloan':
       ...
       if '/' in _dayobs:
          _dayobs = '19'+''.join(_dayobs.split('/')[::-1])
       ...
       _filter = hdr.get('HIERARCH FPA.FILTER').split('.')[0]
...
def downloadPS1(homedir,filename):
    ...
    try:
        ...
        for i in range(0,len(_index)):
            ...
            if  _index[i].split('|')[0].count('.fits') and _index[i].split('|')[0].count('results.fits')==0:

                urlOfFileToDownload3 = dataStoreBase + dataStoreProduct +_index[i].split('|')[0]
                ...
                os.system('mv '+localFilename3.split('/')[-1]+' '+homedir+filename+'/')
                if 'unconv.fits' in localFilename3.split('/')[-1]:
                           frames.append(homedir+filename++'/'+localFilename3.split('/')[-1])
    except:
