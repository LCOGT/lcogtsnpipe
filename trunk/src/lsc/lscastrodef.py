...
#######
        ...
        for i in range(0,len(stdcoo['ra'])):
            ...
            if  stdcoo['ra'][i].count(':'):
                stdcoo['ra'][i]=(int(stdcoo['ra'][i].split(':')[0])+float(stdcoo['ra'][i].split(':')[1])/60+float(stdcoo['ra'][i].split(':')[2])/3600.)*15
                if str(stdcoo['dec'][i]).count('-')==0:   
                    stdcoo['dec'][i]=int(stdcoo['dec'][i].split(':')[0])+float(stdcoo['dec'][i].split(':')[1])/60+float(stdcoo['dec'][i].split(':')[2])/3600.
                else:
                    stdcoo['dec'][i]=(-1)*(abs(int(stdcoo['dec'][i].split(':')[0]))+float(stdcoo['dec'][i].split(':')[1])/60+float(stdcoo['dec'][i].split(':')[2])/3600.)
    ...
    for i in range(len(xusno2_new)):
            ...
            if dist[idist]<max_sep:
                ...
                if imex:
                    ...
                    try:
                        _fwhm2=median(compress(array(ime[3].split()[-3:],float)<99,(array(ime[3].split()[-3:],float))))
                        fwhm2.append(_fwhm2)
                    except: pass
    if len(xref)>=nummin:
            ...
            if 'rms' in _ccmap1[_ccmap1.index('Wcs mapping status')+1]:
                try:           rmsx,rmsy=array(_ccmap1[_ccmap1.index('Wcs mapping status')+1].split(':')[-1].split()[0:2],float)
                except:        rmsx,rmsy=array(_ccmap1[_ccmap1.index('Wcs mapping status')+1].split(':')[-1].split()[0:2])
...
def querysloan(ra1,dec1,radius,mr1,mr2):
    ...
    for i in righe[1:]:
        if len(i.split(','))==14:
            _id0,_ra0,_dec0,_u0,_g0,_r0,_i0,_z0,_du0,_dg0,_dr0,_di0,_dz0,_type0=i.split(',')
            if mr1 and mr2:
...
def readapass(_ra,_dec,radius=30,field=''):
    ...
    print( yyy[0])
    for i in range(0,len(yyy[0].split())):
        if yyy[0].split()[i] in ['#RAdeg', 'DECdeg', 'B', 'V', 'Pg', 'Pr', 'Pi', 'eB', 'eV', 'eg', 'er', 'ei']:
            column[i]=yyy[0].split()[i]
    print(column)
    zzz=[]
    for j in yyy:
        if j[0]!='#':
...
def finewcs(img):
    ...
    if len(pos0)>5:
        for ff in fitgeo:
            ...
            if 'rms' in _ccmap1[_ccmap1.index('Wcs mapping status')+1]:
                try:           rmsx,rmsy=np.array(_ccmap1[_ccmap1.index('Wcs mapping status')+1].split(':')[-1].split()[0:2],float)
                except:        rmsx,rmsy=np.array(_ccmap1[_ccmap1.index('Wcs mapping status')+1].split(':')[-1].split()[0:2])
            print(rmsx,rmsy,ff)
            if rmsx<2 and rmsy<2:
...
def run_astrometry(im, clobber=True,redo=False):
    ...
    if done:
        ...
        if os.path.exists('tmpwcs.fits'):
            ...
            lsc.mysqldef.updatevalue('photlco', 'WCS', 0, im.split('/')[-1])
        else:
