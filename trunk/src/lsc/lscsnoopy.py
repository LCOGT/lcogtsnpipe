...
#    iraf.noao.digiphot.daophot.centerpars.cbox = 0
    ...
    for i in tmptbl:
        try:
            fitmag.append(float(i.split()[0]))#-2.5*log10(_exptime))
        except:
            fitmag.append(i.split()[0])
        try:
            magerr.append(float(i.split()[1]))
        except:
            magerr.append(i.split()[1])
        centx.append(float(i.split()[2]))
        centy.append(float(i.split()[3]))
    tmptbl=iraf.txdump("apori","mag",expr='yes', Stdout=1)
    apori1,apori2,apori3=[],[],[]
    for i in tmptbl:
        try:
            apori1.append(float(i.split()[0]))#-2.5*log10(_exptime))
        except:
            apori1.append(i.split()[0])
        try:            
            apori2.append(float(i.split()[1]))#-2.5*log10(_exptime))
        except:
            apori2.append(i.split()[1])
        try:
            apori3.append(float(i.split()[2]))#-2.5*log10(_exptime))
        except:
            apori3.append(i.split()[2])
    ...
    for i in range(len(tmptbl)):
        try:
            apmag1.append(float(tmptbl[i].split()[0]))#-2.5*log10(_exptime))
        except:
            apmag1.append(9999)
        try:
            apmag2.append(float(tmptbl[i].split()[1]))#-2.5*log10(_exptime))
        except:
            apmag2.append(9999)
        try:
            apmag3.append(float(tmptbl[i].split()[2]))#-2.5*log10(_exptime))
        except:
            apmag3.append(9999)
        try:
            dapmag1.append(float(tmptbl[i].split()[3]))#-2.5*log10(_exptime))
        except:
            dapmag1.append(9999)
        try:
            dapmag2.append(float(tmptbl[i].split()[4]))#-2.5*log10(_exptime))
        except:
            dapmag2.append(9999)
        try:
            dapmag3.append(float(tmptbl[i].split()[5]))#-2.5*log10(_exptime))
        except:
            dapmag3.append(9999)
        try:
...
def errore(img,imgpsf,coordlist,size,truemag,fwhm0,leng0,_show,_interactive,_numiter,z11,z22,midpt,nax,nay,xbgord0,ybgord0,_recenter,apco0,dmax,dmin):
    ...
    while dartf>=size-1:
        if _interactive:
            artfac0 =input('>>> Dispersion of artificial star positions (in units of FWHM) [1] ')
    ...
    while i<=8:
        ...
        ss=ff.readline()
        ff.close()
        xbb=float(ss.split()[0])
        ybb=float(ss.split()[1])
