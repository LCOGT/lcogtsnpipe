...
#                             if fn.replace('.optimal', '').count('.') == 3 else inst[0:2]
    ...
    if classid is not None:
        standards = lsc.mysqldef.query(['select id from targets where classificationid='+str(classid)], conn)
...
def plotfast2(setup):
    ...
    def delete_hook(i):
        lsc.mysqldef.query(['update photlco set psfmag=9999, psfdmag=9999, apmag=9999, dapmag=9999, mag=9999, dmag=9999 where filename="{}"'.format(filenames[i])], conn)
...
def get_standards(epoch, name, filters, standard_name='all', match_by_site=False):
    ...
    matching_stds = lsc.mysqldef.query([query], conn)
    if matching_stds:
