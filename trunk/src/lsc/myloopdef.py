...
def run_getmag(imglist, _output='', _interactive=False, _show=False, _bin=1e-10, magtype='mag', snex2_upload=False, database='photlco'):
    ...
    if _output:
        ...
        if snex2_upload:
            ...
            if int(ftype)==1:
                phot_type = input('The default photometry type for non-difference imaging is PSF. If this is not PSF photometry please enter "Aperture", "Mixed", or "Unsure": ')
                ...
                phot_type = input('The default photometry type for difference imaging is Aperture. If this is not Aperture photometry please enter "PSF", "Mixed", or "Unsure": ')
                ...
                template_source = input('Please enter the source of the template used to perform background subtraction (i.e. LCO or SDSS): ')
                upload_extras['template_source'] = template_source

            ### Prompt user for inputs for a few upload extras
            reducer_group = input('Please enter the group reducing this data (i.e. LCO, UC Davis, etc.): ')
            if reducer_group!='LCO':
                upload_extras['reducer_group'] = reducer_group

            final_photometry = input('Is this reduction final? [y/n]')
            if final_photometry == 'y':
                upload_extras['final_reduction'] = True
            else:
                upload_extras['final_reduction'] = False
            used_in = input('Will this data be used in a paper? If so, please enter the name of the first author like "Last Name, First Name": ')
            if used_in:
            ...
            ### Upload to snex2
            username = input('Please enter your SNEx2 username: ')
        ...
        if status >= 1:
            ...
            if os.path.isfile(catfile):
                ...
                if len(lines) > 2:
                    ...
                    aa = input('>>>good catalogue [[y]/n] or [b] bad quality ? ')
                    if not aa: aa = 'y'
    ...
    for img in imglist:
        ...
        if status >= 1:
            ...
            if os.path.isfile(_dir + img.replace('.fits', '.psf.fits')):
                ...
                aa = input('>>>good psf [[y]/n] or [b] bad quality ? ')
                if not aa: aa = 'y'
                if aa in ['n', 'N', 'No', 'NO', 'bad', 'b', 'B']:
...
def checkwcs(imglist, force=True, database='photlco', _z1='', _z2=''):
    ...
    for img in imglist:
        ...
        if status >= 0 or force == False:
            ...
            aa = input('>>>good wcs [[y]/n] or [b] bad quality ? ')
            if not aa:
                aa = 'y'
            if aa in ['n', 'N', 'No', 'NO', 'bad', 'b', 'B']:
                print('updatestatus bad')
...
def checkfast(imglist, force=True, database='photlco'):
    ...
    for img in imglist:
        ...
        if status >= 0 or force == False:
            ...
            iraf.display(_dir + img + '[0]', 1, fill=True, Stdout=1)
            ###########################################
            aa = input('>>>good or bad quality [[g]/b]? ')
            if not aa: aa = 'g'
            if aa in ['bad', 'b', 'B']:
                lsc.mysqldef.updatevalue(database, 'wcs', 9999, os.path.basename(img))
                lsc.mysqldef.updatevalue(database, 'psf', 'X', os.path.basename(img))
    ...
    for img in imglist:
        ...
        if status >= 0:
            ...
            if os.path.isfile(origimg) and os.path.isfile(maskimg):
                ...
                ans = input('>>> good mask [[y]/n] or [b]ad quality? ')
    ...
    for img in imglist:
        ...
        if status >= 0:
            ...
            if os.path.isfile(diffimg) and os.path.isfile(origimg) and os.path.isfile(tempimg):
                ...
                ans = input('>>> good difference [[y]/n] or [b]ad quality (original image)? ')
                if ans in ['n', 'N', 'No', 'NO', 'bad', 'b', 'B']:
                    print('updatestatus bad')
                    print('rm', diffimg.replace('.fits', '*'))
                    os.system('rm ' + diffimg.replace('.fits', '*'))
...
def checkmag(imglist, datamax=None):
    ...
    for img in imglist:
        ...
        if status > 1:
            ...
            aa = input('>>>good mag [[y]/n] or [b] bad quality ? ')
            if aa in ['n', 'N', 'No', 'NO', 'bad', 'b', 'B']:
...
def plotfast(setup, output='', database='photlco'):  #,band,color,fissa=''):
    ...
    if not output:
        plt.ion()
        plt.draw()
        input('press d to mark. Return to exit ...\n')
        plt.close()
    else:
